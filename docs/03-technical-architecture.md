...
| `DUoM Item Tracking Lines` | 50112 | `Item Tracking Lines` | Muestra DUoM Ratio y DUoM Second Qty en el repeater de seguimiento de lotes. `DUoM Second Qty.OnValidate`: llama a `NormalizeTrackingDUoMSecondQty` (recalcula DUoM Ratio en modos Variable/AlwaysVariable) y luego a `ValidateTrackingSpecLine` para feedback inmediato. `DUoM Ratio.OnValidate`: llama directamente a `ValidateTrackingSpecLine`. `OnQueryClosePage` (OK/LookupOK): **(1) `ValidateTrackingSpecBufferEachLine` — primera barrera de cierre: valida el ratio de coherencia DUoM de cada línea funcional del buffer (líneas vacías/de inserción ignoradas); bloquea con error si alguna línea tiene DUoM Ratio ≠ DUoM Second Qty / Quantity (Base); (2)** `SyncPurchLineFromTrackingBuffer` — actualiza la Purchase Line con el total agregado del tracking (fuente de verdad → resumen agregado); **(3)** `ValidateTrackingSpecBufferForPurchLine` — sanity check post-sync (siempre pasa tras sync). La cancelación no queda bloqueada. **Seguridad del cursor (bugfix T-REOPEN-05):** todos los helpers usan internamente `LocalTrackingSpec.Copy(Rec, true)` para iterar con cursor independiente, sin tocar el cursor del `Rec` del page; esto previene que BC registre duplicados al cerrar. (Issues 22, close-validation, sync-tracking, T-REOPEN-05, validación-temprana-DUoM) |
...
| `DUoM Inventory Subscribers` | 50104 | Subscribers para diario de productos / ILE / líneas de documentos registrados. Proyección de compra: usa `OnBeforeItemJnlPostLine` en `Purch.-Post` (se dispara después de que BC asigna `ItemJnlLine.Quantity := QtyToBeReceived`, garantizando que `CalcProjectedSecondQty` recibe la cantidad real de posting). Proyección de venta: usa `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Sales-Post` (se dispara después de que BC asigna `ItemJnlLine.Quantity := -QtyToBeShipped`). La diferencia de eventos entre compra y venta refleja que en `Purch.-Post` la asignación de cantidad ocurre DESPUÉS de `CopyDocumentFields`, mientras que en `Sales-Post` ocurre ANTES. |
| `DUoM Doc Transfer Helper` | 50105 | Helper centralizado de copia/proyección de campos DUoM entre líneas de documento. En posting sin tracking, proyecta `Purchase Line` / `Sales Line` hacia `Item Journal Line` con prorrateo proporcional sobre la cantidad efectivamente registrada y con el signo técnico del movimiento tomado del propio IJL. |
