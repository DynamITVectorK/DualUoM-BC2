...
| `DUoM Item Tracking Lines` | 50112 | `Item Tracking Lines` | Muestra DUoM Ratio y DUoM Second Qty en el repeater de seguimiento de lotes. `OnValidate` en ambos campos llama a `DUoM Tracking Coherence Mgt` (50111) para feedback inmediato al usuario. `OnQueryClosePage` valida el total DUoM agregado contra la Purchase Line origen al confirmar con OK/LookupOK; la validación usa el buffer temporal `Tracking Specification` (pre-persist) y llama a `ValidateTrackingSpecBufferForPurchLine` (50111). La cancelación no queda bloqueada. (Issue 22, Issue coherencia, Issue close-validation) |
...
| `DUoM Tracking Coherence Mgt` | 50111 | **Validación centralizada de coherencia DUoM** entre línea de compra y líneas de seguimiento (`Reservation Entry`). Métodos públicos: `ValidatePurchLineTrackingCoherence` (compara suma DUoM de tracking con `Purchase Line.DUoM Second Qty`), `ValidateTrackingSpecBufferForPurchLine` (valida el total DUoM agregado del buffer temporal `Tracking Specification` contra la `Purchase Line` origen — usado por `OnQueryClosePage` de `DUoM Item Tracking Lines`), `ValidateTrackingSpecLine` (valida ratio y modo en un `Tracking Specification` individual, usado por la page extension para feedback inmediato), `CalcTrackingDUoMTotalsForPurchLine` (suma DUoM Second Qty de `Reservation Entry`), `AssertRatioCoherence` (valida `|BaseQty × Ratio − SecondQty| ≤ Precision`), `GetDUoMRoundingPrecision`, `GetExpectedRatio`. La validación se ejecuta en DOS puntos (arquitectura de dos barreras): (1) page `DUoM Item Tracking Lines` (50112) vía `OnQueryClosePage` para bloquear el cierre con datos DUoM incoherentes (usa buffer temporal pre-persist); (2) `DUoM Purchase Subscribers` (50102) vía `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Purch.-Post` para bloqueo server-side antes del posting (usa `Reservation Entry` persistida). |
