...
| `DUoM Item Tracking Lines` | 50112 | `Item Tracking Lines` | Muestra DUoM Ratio y DUoM Second Qty en el repeater de seguimiento de lotes. `OnValidate` en ambos campos llama a `DUoM Tracking Coherence Mgt` (50111) para feedback inmediato al usuario. (Issue 22, Issue coherencia) |
...
| `DUoM Tracking Coherence Mgt` | 50111 | **Validación centralizada de coherencia DUoM** entre línea de compra y líneas de seguimiento (`Reservation Entry`). Métodos públicos: `ValidatePurchLineTrackingCoherence` (compara suma DUoM de tracking con `Purchase Line.DUoM Second Qty`), `ValidateTrackingSpecLine` (valida ratio y modo en un `Tracking Specification` individual, usado por la page extension para feedback inmediato), `CalcTrackingDUoMTotalsForPurchLine` (suma DUoM Second Qty de `Reservation Entry`), `AssertRatioCoherence` (valida `|BaseQty × Ratio − SecondQty| ≤ Precision`), `GetDUoMRoundingPrecision`, `GetExpectedRatio`. La validación se ejecuta en dos puntos: (1) page `DUoM Item Tracking Lines` (50112) vía `OnValidate` para feedback inmediato; (2) `DUoM Purchase Subscribers` (50102) vía `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Purch.-Post` para bloqueo server-side antes del posting. |
