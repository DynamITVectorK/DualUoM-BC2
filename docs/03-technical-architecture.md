...
| `DUoM Purchase Subscribers` | 50102 | Subscribers de eventos del flujo de compras (Quantity y Variant Code en Purchase Line). Incluye validación de coherencia DUoM pre-posting vía `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Purch.-Post`, que delega en `DUoM Tracking Coherence Mgt` (50111) antes de crear ningún ILE. |
...
| `DUoM Doc Transfer Helper` | 50105 | Helper centralizado para propagación DUoM: copia entre líneas de documento registrado y preparación de `Item Journal Line` desde Purchase/Sales antes del posting |
