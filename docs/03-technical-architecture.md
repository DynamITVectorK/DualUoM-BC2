...
| `DUoM Tracking Coherence Mgt` | 50111 | **Gestión centralizada de validación/coherencia DUoM** para edición de tracking y flujos de documento. Métodos públicos principales: `NormalizeTrackingDUoMSecondQty`, `NormalizeTrackingQuantityBase`, `ValidateTrackingSpecLineForFieldEdit`, `ValidateTrackingSpecLine`, `ValidateTrackingSpecBufferForSalesLine`, `ValidatePurchLineTrackingCoherence`, `ValidateSalesLineTrackingCoherence`, `CalcTrackingDUoMTotalsForPurchLine`, `AssertRatioCoherence`, `GetDUoMRoundingPrecision`, `GetExpectedRatio`. Para ventas valida tanto en buffer (`Tracking Specification`) como en guard server-side (`Reservation Entry`) usando filtros estándar por origen. |
