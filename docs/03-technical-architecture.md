...
| `DUoM Item Trk Summary` | 50130 | `Item Tracking Summary` | Muestra DUoM Ratio y DUoM Second Qty en `Seg. productos - Selec. movs.` y expone el total DUoM secundario seleccionado en la sesión. `OnAfterGetRecord` llama a `DUoM Entry Summary Mgt.PopulateDUoMForEntrySummary` para poblar los campos DUoM en cada línea durante la carga inicial del buffer, usando `Total Available Quantity` cuando `Selected Quantity = 0`. |
