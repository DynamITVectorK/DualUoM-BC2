...
|---|---|---|---|
| `Item.TableExt` | 50100 | `Item` | Cascade-delete del registro DUoM Item Setup al borrar el artículo + FlowField `DUoM Inventory` = `SUM(Item Ledger Entry."DUoM Second Qty")` con filtros estándar equivalentes al patrón de inventario BC |
...
| `DUoM Item Trk Summary` | 50130 | `Item Tracking Summary` | Muestra DUoM Ratio y DUoM Second Qty en `Seg. productos - Selec. movs.` y expone el total DUoM secundario seleccionado en la sesión. `OnAfterGetRecord` y `OnAfterGetCurrRecord` llaman a `DUoM Entry Summary Mgt.PopulateDUoMForEntrySummary` para poblar/refrescar los campos DUoM en cada línea durante la carga y al posicionarse en el registro actual, usando `Total Available Quantity` cuando `Selected Quantity = 0`. |
...
| `DUoM Purchase Subscribers` | 50102 | Subscribers de eventos del flujo de compras (`No.`, `Variant Code`, `Unit of Measure Code`, `Quantity` en Purchase Line). Recalcula coherencia DUoM en entrada de datos (Fixed/Variable) y preserva valores manuales en AlwaysVariable cuando solo cambia la cantidad. Incluye validación de coherencia DUoM pre-posting vía `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Purch.-Post`, que delega en `DUoM Tracking Coherence Mgt` (50111) antes de crear ningún ILE. |
| `DUoM Sales Subscribers` | 50103 | Subscribers de eventos del flujo de ventas (`No.`, `Variant Code`, `Unit of Measure Code`, `Quantity` en Sales Line) con la misma política de coherencia en entrada de datos que compras. Incluye validación de coherencia DUoM pre-posting vía `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Sales-Post`, delegando en `DUoM Tracking Coherence Mgt` (50111). |
| `DUoM Inventory Subscribers` | 50104 | Subscribers para diario de productos / ILE / líneas de documentos registrados. Proyección compra: `OnBeforeItemJnlPostLine` en `Purch.-Post` (tras asignación de `ItemJnlLine.Quantity`). Proyección venta: `OnPostItemJnlLineOnAfterCopyDocumentFields` en `Sales-Post` (tras asignación de `ItemJnlLine.Quantity`). |
| `DUoM Doc Transfer Helper` | 50105 | Helper centralizado de copia/proyección de campos DUoM entre líneas de documento. En posting sin tracking, proyecta con prorrateo proporcional sobre la cantidad efectivamente registrada: `ProjectPurchLineToItemJnlLine` mantiene el signo técnico según IJL y `ProjectSalesLineToItemJnlLine` fuerza signo negativo de salida al proyectar desde `Sales Line` hacia `Item Journal Line`. |
...
| `DUoM Tracking Coherence Mgt` | 50111 | **Gestión centralizada de validación/coherencia DUoM** para edición de tracking y flujos de documento. Métodos públicos principales: `NormalizeTrackingDUoMSecondQty`, `NormalizeTrackingQuantityBase`, `ValidateTrackingSpecLineForFieldEdit`, `ValidateTrackingSpecLine`, `ValidateTrackingSpecBufferForSalesLine`, `ValidatePurchLineTrackingCoherence`, `ValidateSalesLineTrackingCoherence`, `CalcTrackingDUoMTotalsForPurchLine`, `AssertRatioCoherence`, `GetDUoMRoundingPrecision`, `GetExpectedRatio`. En ventas: al cerrar tracking, sincroniza `Sales Line` desde el total real del buffer en modos `Variable`/`AlwaysVariable`; en `Fixed` mantiene validación estricta de igualdad línea vs tracking. |
...
| `DUoM Entry Sum Subscribers` | 50131 | Suscriptores `OnAfterValidateEvent` en `Entry Summary` para `Lot No.`, `Serial No.` y `Selected Quantity`. Aplica ratio DUoM por lote/serie y recalcula `DUoM Second Qty` en la selección de movimientos. Usa `DUoM Entry Summary Mgt.` (50132) para resolver el contexto de artículo/variante. |
| `DUoM Entry Summary Mgt.` | 50132 | Helper centralizado para resolver el contexto de artículo/variante desde un registro `Entry Summary`. `TryResolveItemContext` usa primero `Table ID` + `Entry No.` para buscar en `Item Ledger Entry` (Table ID = 32), `Reservation Entry` (Table ID = 337) o `Tracking Specification` (Table ID = 6500). Si ese camino no resuelve (p.ej. `Tracking Specification` temporal no persistido), aplica **último recurso read-only para UI**: fallback por `Item Ledger Entry` filtrado por `Lot No.`/`Serial No.` con `Open = true` y `Remaining Quantity <> 0`, aceptando solo contexto **único** (`Item No.` + `Variant Code`). Si hay ambigüedad, devuelve `false` y no puebla DUoM. Necesario porque `Entry Summary` (tabla 338) no expone `Item No.` ni `Variant Code` en BC 27. `PopulateDUoMForEntrySummary` rellena `DUoM Ratio` y `DUoM Second Qty` en una línea de `Entry Summary`; cuando `Selected Quantity = 0` usa `Total Available Quantity` como base del cálculo, y cuando `Selected Quantity ≠ 0` usa `Abs(Selected Quantity)`. Llamado desde `DUoM Item Trk Summary` (50130) en `OnAfterGetRecord` y `OnAfterGetCurrRecord` para poblar y refrescar los campos en la UI. |

---

## Fuente de verdad DUoM por nivel (Issue sync-tracking)

```
Item Tracking Lines (TrackingSpec buffer) = edición/visualización temporal por lote
                       ↓ patrón estándar BC de tracking
Reservation Entry      = persistencia operativa por lote (documento vivo)
                       ↓ buffers temporales de tracking + split de posting
Item Journal Line      = origen inmediato de DUoM en contabilización
                       ↓ NormalizeILESign (DUoM Sign Mgt)
Item Ledger Entry      = verdad histórica contabilizada por movimiento/lote
```

**Regla de sincronización:**
- No existe sincronización manual `Tracking → Purchase Line` en el patrón vigente.
...
- En ventas, al cerrar `Item Tracking Lines`, la `Sales Line` se sincroniza con el agregado real
  del tracking en modos `Variable`/`AlwaysVariable` (`DUoM Second Qty` y `DUoM Ratio`).
- En ventas modo `Fixed`, el cierre/guard de coherencia sigue bloqueando si el total tracking no
...
**NORMA OBLIGATORIA**: `ILE."DUoM Second Qty"` **siempre** recibe sus datos del IJL.
Nunca se calcula desde campos del ILE. La fórmula canónica es:
```al
ILE."DUoM Second Qty" := DUoMSignMgt.NormalizeILESign(ILE, IJL."DUoM Second Qty");
```
`NormalizeILESign()` aplica el signo correcto según el movimiento del ILE.

**SIN Item Tracking** (artículos sin lotes):
```
IJL (DUoM Ratio del artículo/variante, DUoM Second Qty ya calculado)
  → Codeunit "Item Jnl.-Post Line" · OnAfterInitItemLedgEntry   [50104]
      ↓ ILE.DUoM Ratio = IJL.DUoM Ratio
      ↓ ILE.DUoM Second Qty = NormalizeILESign(ILE, IJL.DUoM Second Qty)
Item Ledger Entry  ✓
```

**CON Item Tracking** (por lote — patrón de `Package Management (6516)`):
```
Tracking Specification (con DUoM Ratio y DUoM Second Qty del lote)
  → Table "Item Journal Line" · OnAfterCopyTrackingFromSpec      [50110]
      ↓ IJL.DUoM Ratio = ratio del lote (si TrackingSpec.DUoM Ratio ≠ 0)
      ↓ IJL.DUoM Second Qty = DUoM Second Qty del lote
Item Journal Line  (ya tiene los valores correctos del lote)
  → Codeunit "Item Jnl.-Post Line" · OnAfterInitItemLedgEntry   [50104]
      ↓ copia ratio + normaliza signo desde IJL (sin recalcular ratio)
...
desde IJL y normalizan el signo mediante `DUoM Sign Mgt`.
...
- El IJL (ya dividido por lote por BC) es la fuente inmediata del ILE: `ILE.DUoM Second Qty`
  **nunca** se calcula desde `ILE.Quantity`. Se toma del IJL y se normaliza el signo mediante
  `DUoM Sign Mgt.NormalizeILESign(...)`.
...
- **No usar `ILE.Quantity` como base de cálculo de `ILE.DUoM Second Qty`.** El IJL (split
  por lote) ya contiene la cantidad correcta del lote. Calcular desde `ILE.Quantity`
  viola la norma ILE←IJL y produce inconsistencias en modo AlwaysVariable.
...
- **Issue 25:** patrón Clear/Blank completado — `OnAfterClearTracking` en `Tracking Specification`
  ...
  Garantiza que DUoM Ratio y DUoM Second Qty se limpian al reinicializar líneas de tracking
  y viajan correctamente en flujos de reclasificación, transferencia y devolución.
...
- **Bug fix (sign + undo flow):** la gestión de signo DUoM se centraliza en `DUoM Sign Mgt`
  (`NormalizeILESign`, `ApplyUndoPurchReceiptSign`, `ApplyUndoSalesShptSign`,
  `ApplyCorrectionILESign`). Los subscribers finales de ILE/VE toman magnitud desde IJL y
  normalizan signo con `DUoM Sign Mgt`, sin recalcular desde `ILE.Quantity`.
  Pruebas añadidas: `DUoM Undo Rcpt Shpt Tests` (50227), T-UNDO-01..05.

---

## Persistencia DUoM en Item Tracking Lines (patrón vigente)

La persistencia DUoM en tracking sigue el patrón estándar de Business Central, alineado
con Piezas:

```
Tracking Specification (buffer)
  → Reservation Entry (persistencia por lote)
  → buffers temporales de tracking/posting
  → Item Journal Line (split por lote)
  → Item Ledger Entry
```

Reglas vigentes:
- No existe lógica DUoM en eventos de cierre de página (`OnBeforeClosePage`/`OnQueryClosePage`).
- No existe sincronización manual `Tracking → Purchase Line`.
- No existe persistencia manual tipo `PersistDUoMToReservEntries`.
- La persistencia y recarga se apoyan en eventos estándar: `OnAfterMoveFields`,
  `OnCreateReservEntryExtraFields`, `OnAfterCopyTrackingFromTrackingSpec`,
  `OnAfterCopyTrackingFromReservEntry` y eventos de carga de buffer.
...
La lógica vigente del flujo tracking implementa cuatro sub-casos según la presencia
de ratio de lote en `DUoM Lot Ratio` (50102), ratio manual en `IJL.DUoM Ratio`
y asignación de lote. La magnitud se resuelve en `IJLCopyTrackingFromSpec` (50110)
y el ILE final solo copia desde IJL con signo normalizado:

| Caso | `DUoM Lot Ratio` (50102) | `IJL.DUoM Ratio` | `IJL.Lot No.` | `ILE.DUoM Second Qty` | Test |
|------|--------------------------|------------------|----------------|------------------------|------|
| 1 | ✅ Existe | — | Cualquiera | El split IJL usa `ratio_lote`; el ILE copia desde IJL | T08–T09 |
| 2 | ❌ No existe | ≠ 0 (manual) | ✅ Asignado | El split IJL usa `ratio_manual`; el ILE copia desde IJL | T14 |
| 3 | ❌ No existe | 0 | ❌ Vacío | `IJL.DUoM Second Qty` (copia) | — |
| 4 | ❌ No existe | 0 | ✅ Asignado | `0` (distribución imposible) | T10 |

**Resumen de implementación vigente:**

```al
// 1) IJLCopyTrackingFromSpec resuelve la magnitud del split
// 2) ILECopyTrackingFromItemJnlLine copia desde IJL
