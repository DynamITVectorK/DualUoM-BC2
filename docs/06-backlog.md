...
> **Última actualización:** 2026-05-03 (Issue close-validation: bloquear cierre de Item Tracking Lines cuando el total DUoM de tracking no coincide con la Purchase Line — `OnQueryClosePage` en pageextension 50112 + método `ValidateTrackingSpecBufferForPurchLine` en codeunit 50111 + 6 tests en codeunit 50222)
...
| close-validation | fix: bloquear cierre de `Item Tracking Lines` con OK cuando el total DUoM del tracking no coincide con la `Purchase Line`. `OnQueryClosePage` en pageextension 50112 delega en `ValidateTrackingSpecBufferForPurchLine` (50111). 6 tests en codeunit 50222 `DUoM Purch Tracking Close`. |
